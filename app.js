        ...
        (()=>{
            ...
            .then(function (registryResponse) {
                ...
                request.post({
                    ...
                    const convertResult = spawnSync('avconv', ['-i ' + __dirname+'/tmp/' + timeStamp + '.ogg' + ' -map 0:a -codec:a opus -b:a 100k -vbr on ' + __dirname + '/tmp/' + timeStamp + 'converted.opus'])
                    console.log(convertResult)
                    fs.writeFileSync(__dirname+'/tmp/' + timeStamp + 'converted.opus', convertResult.stderr)
                    fs.readdirSync(__dirname+'/tmp/').forEach(file => {
                        console.log(file);
                    });
                    const exist = fs.existsSync(__dirname+'/tmp/' + timeStamp + 'converted.opus')
                    console.log("Converted Exist", exist)
                    messengerService.sendFile(user, __dirname + '/tmp/' + timeStamp + 'converted.opus')
                    fs.unlinkSync(__dirname+'/tmp/' + timeStamp + '.ogg')
                    fs.unlinkSync(__dirname+'/tmp/' + timeStamp + 'converted.opus')
                });
